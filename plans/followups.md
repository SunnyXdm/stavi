...
## From Phase 8f
- **`useOrchestration.ts` and `ai/index.tsx` pre-existing over 400 lines**: Both files were 568 and 837 lines before Phase 8f. My additions (+72 each) push them to 640 and 896. Splitting `useOrchestration.ts` is deferred to Phase 8g — candidate seams: `createNewChat`/`ensureActiveThread` into a `thread-actions.ts` helper, and `processEventInner` into an `event-reducer.ts`. `ai/index.tsx` could split the model-selection logic into a separate file.
...
- **`createNewChat` does not propagate the current model selection**: When the sidebar "New Chat" button creates a thread, the `agentRuntime` is undefined (falls back to workspace default). Phase 8f wires the button; wiring the model selection from the current composer state is deferred to Phase 8g polish.
